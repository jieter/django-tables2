...
class DynamicColumnsTest(TestCase):
    def test_dynamically_adding_columns(self):
        """
        Test adding columns dynamically (#403).


        When adding columns to self.base_columns, they were actually added to the class attribute `Table.base_columns`,
        and not to the instance attribute, `table.base_columns`.
        """
        ...
        self.assertEqual(
            list(
                MyTable(data, extra_columns=[("country", tables.Column())]).columns.columns.keys()
            ),
            ["name", "country"],
        )
    ...
    def test_dynamically_removing_columns(self):
        ...
        self.assertEqual(
            list(
                MyTable(
                    data, extra_columns=[("country", tables.Column()), ("name", None)]
    ...
    def test_sorting_on_dynamically_added_columns(self):
        ...
        table = MyTable(
            data,
            order_by="-country",
            extra_columns=[("country", tables.Column(verbose_name=_("country")))],
        )
    ...
    def test_dynamically_add_column_with_sequence(self):
        ...
        self.assertEqual(
            list(
                MyTable(
                    data, extra_columns=[("country", tables.Column())], sequence=("name", "...")
                ).columns.columns.keys()
            ),
            ["name", "country"],
        )
    ...
    def test_sequence_and_extra_columns(self):
        """The exact moment the '...' is expanded is crucial here (#486)."""
    ...
    def test_change_attributes(self):
        """Test attributes of columns can be changed #574)."""
